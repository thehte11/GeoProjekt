...
<meta charset="UTF-8">
    ...
    <style>
        ...
        /* Styling für die Suchergebnisse im Panel */
        #search-results {
            display: none;
            margin-top: 20px;
        }
...
<body>
    ...
    <script>
        ...
        function getCountryInfo(countryDe) {
            ...
            return {
                mining: Array.from(miningMinerals),
                manufacturing: manufacturingComps,
                components: allComps
            };
        }
        ...
        function getMapDataForMaterial(materialName) {
            ...
            return {
                miningLocations: Array.from(miningSet),
                manufacturingLocations: Array.from(manufacturingSet)
            };
        }
        ...
        function highlightRoles(miningArray, manufacturingArray) {
            ...
            g.selectAll("path").each(function(d) {
                ...
                if (isMining && isManufacturing) {
                    d3.select(this).classed("highlighted-both", true).raise();
                } else if (isMining) {
                    d3.select(this).classed("highlighted-mining", true).raise();
                } else if (isManufacturing) {
                    d3.select(this).classed("highlighted-manufacturing", true).raise();
                }
            });
        ...
        d3.json("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson").then(function(data) {
            ...
            g.selectAll("path")
                ...
                .on("click", function(event, d) {
                    const countryEn = d.properties.name;
                    const countryDe = countryTranslations[countryEn] || countryEn;
                    document.getElementById("country-search").value = countryDe;
